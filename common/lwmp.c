...
const uint64_t lwmp_init_payload_size = LWMP_MAX_PDU_SIZE - lwmp_hdr_size;

const uint32_t resp_codes[] =
{
#define X_RESP_CODES(enumname, respname, respval) [enumname] = respval,
...
uint64_t lwmp_stream_resync(unsigned char* buf, uint64_t buf_size)
{
    ...
    while (true)
    {
        void* hdr_mark = memmem(buf, buf_size, &be_hdr_mark, sizeof(be_hdr_mark));
        if (!hdr_mark)
            return 0;

        new_size = (void*)(buf + buf_size) - hdr_mark;
        memmove(buf, hdr_mark, new_size);
        buf_size = new_size;

        if (new_size >= LWMP_HDR_SIZE)
        {
            uint32_t hdr_crc32 = crc32(buf, offsetof(lwmp_pdu, crc32));
            uint32_t orig_crc32;
            memcpy(&orig_crc32, buf + offsetof(lwmp_pdu, crc32), sizeof(uint32_t));
            if (htonl(hdr_crc32) != orig_crc32)
            {
                buf[0] = 0;
                continue;
            }
...
{
    if (optdata_len > OPTDATA_LEN)
    {
        dzlog_error("optdata_len may not be longer than OPTDATA_LEN(32)");
        return;
    }
    ...
    if (optdata)
    {
        ...
        memset(pdu->optional_data + optdata_len, 0, OPTDATA_LEN - optdata_len);
    }
    else
    {
        memset(pdu->optional_data, 0, OPTDATA_LEN);
    }
